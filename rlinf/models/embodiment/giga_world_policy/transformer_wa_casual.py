...
class CasualWorldActionTransformer(
    ...
    def _forward_inference(
        ...
        # 兼容 wrapper 里传入的 latent 排列:
        # 如果 hidden_states 是 [B, W, T, H, C] 或 [B, T, H, W, C] 这种 channel-last 形式，
        # 且最后一维正好是 patch_embedding 期望的 in_channels，就转成 [B, C, T, H, W]
        expected_c = self.patch_embedding.weight.shape[1]

        if hidden_states.ndim == 5 and hidden_states.shape[1] != expected_c and hidden_states.shape[-1] == expected_c:
            # 当前日志里实际是 [B, W, T, H, C] = [1, 100, 2, 12, 48]
            # 转成 [B, C, T, H, W] = [1, 48, 2, 12, 100]
            hidden_states = hidden_states.permute(0, 4, 2, 3, 1).contiguous()
