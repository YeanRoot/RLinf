...
class EmbodiedGigaWAFSDPPolicy(EmbodiedFSDPActor):
    ...
    def setup_gigawa_components(self):
        ...
        self.freeze_actor_updates_for_takeover_test = bool(
            self.cfg.algorithm.get("freeze_actor_updates_for_takeover_test", False)
        )
        self.utd_ratio = int(
    ...
    def update_one_epoch(self, train_actor: bool = True):
        ...
        if actor_update_due and actor_train_enabled:
            ...
            if self.freeze_actor_updates_for_takeover_test:
                metrics_data.update({"gigawa/freeze_actor_updates_for_takeover_test": 1.0})
            else:
                self.optimizer.step()
                self.lr_scheduler.step()
                actor_updated = True
