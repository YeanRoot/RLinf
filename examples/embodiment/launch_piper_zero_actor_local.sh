...
RUN_ROOT="${RUN_ROOT:-$REPO/examples/results/offline_rl_piper_success100_failure100_7_zero_actor_local_$(date +%Y%m%d_%H%M%S)}"
EXP_NAME="piper_zero_actor_local"
CONFIG_NAME="offline_rl_piper_success100_failure100_7_zero_actor_local"
...
printf "%s/%s\n" "$RUN_ROOT" "$EXP_NAME" > "$REPO/examples/results/latest_zero_actor_local_path.txt"

tmux has-session -t piper_local_zrl 2>/dev/null && tmux kill-session -t piper_local_zrl || true
tmux has-session -t piper_local_zviz 2>/dev/null && tmux kill-session -t piper_local_zviz || true

tmux new-session -d -s piper_local_zrl "bash -ic '
  source switch_env gigaworld &&
...
tmux new-session -d -s piper_local_zviz "bash -ic '
  ...
  python -u examples/embodiment/watch_openloop_checkpoints.py \
    ...
    --norm-json \"$NORM_JSON\" \
    --urdf \"$URDF\" \
    --out-root \"$RUN_ROOT/$EXP_NAME/openloop_viz\" \
    --max-files 3 \
    --interval 30 \
    --stop-step 600 \
    --min-step 0 \
...
echo "[launch] train session=piper_local_zrl log=$RUN_ROOT/$EXP_NAME/logs/train.log"
echo "[launch] viz session=piper_local_zviz log=$RUN_ROOT/$EXP_NAME/logs/watch_openloop.log"
